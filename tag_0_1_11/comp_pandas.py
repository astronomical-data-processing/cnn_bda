""" Average BlockVisibility using pandas (RASCIL version tag 0.1.11)
...
from rascil.processing_components.visibility.base import vis_summary
...
def coalesce_visibility(vis: BlockVisibility, time_coal=0.0, frequency_coal=0.0, max_time_coal=100,
    ...
    log.debug('coalesce_visibility: comp_pandas')
...
def average_in_blocks(vis, flags, uvw, wts, imaging_wts, times, integration_time, frequency, channel_bandwidth,
    ...
    :return:
    """
    log.debug('average_in_block: comp_pandas')
    ...
    data_pd = pd.DataFrame({'cindex': cindex_flat,
                            'allpwtsgrid': allpwtsgrid_flat,
                            'ctime': allpwtsgrid_flat * time_grid.flatten(),
                            'cfrequency': allpwtsgrid_flat * frequency_grid.flatten(),
                            'cintegration_time': allpwtsgrid_flat * integration_time_grid.flatten(),
                            'cchannel_bandwidth': allpwtsgrid_flat * channel_bandwidth_grid.flatten()
                            })

    uvw_list = 'u', 'v', 'w'
    for enumerate_id, i in enumerate(uvw_list):
        posi = uvw_grid[:, mask, :, enumerate_id].flatten()
        data_pd[i] = posi * allpwtsgrid_flat

    npol_list = numpy.empty([npol, 4], dtype=object)
    for i in range(npol):
        wts_flat = wts[:, mask, :, i].flatten()
        vis_flat = vis[:, mask, :, i].flatten()
        imaging_wts_flat = imaging_wts[:, mask, :, i].flatten()
        npol_list[i, :] = 'cwts0' + str(i), 'cwts' + str(i), 'cvis' + str(i), 'cimwts' + str(i)
        data_pd[npol_list[i, 0]] = wts_flat
        data_pd[npol_list[i, 1]] = wts_flat * wts_flat
        data_pd[npol_list[i, 2]] = vis_flat * wts_flat
        data_pd[npol_list[i, 3]] = imaging_wts_flat * wts_flat

    # Calculated with gropby and get results
    data_group = data_pd.groupby('cindex').sum()

    allwts = data_group['allpwtsgrid'].values
    ctime = data_group['ctime'].values
    cfrequency = data_group['cfrequency'].values
    cintegration_time = data_group['cintegration_time'].values
    cchannel_bandwidth = data_group['cchannel_bandwidth'].values

    allwts_mask = allwts > 0.0
    ctime[allwts_mask] = ctime[allwts_mask] / allwts[allwts_mask]
    cfrequency[allwts_mask] = cfrequency[allwts_mask] / allwts[allwts_mask]
    cintegration_time[allwts_mask] = cintegration_time[allwts_mask] / allwts[allwts_mask]
    cchannel_bandwidth[allwts_mask] = cchannel_bandwidth[allwts_mask] / allwts[allwts_mask]

    for enumerate_id, i in enumerate(uvw_list):
        cuvw[:, enumerate_id] = data_group[i].values
        cuvw[:, enumerate_id][allwts_mask] = cuvw[:, enumerate_id][allwts_mask] / allwts[allwts_mask]

    for enumerate_id, i in enumerate(npol_list):
        cwts_flag = data_group[i[0]].values
        cwts[:, enumerate_id] = data_group[i[1]].values
        cvis[:, enumerate_id] = data_group[i[2]].values
        cimwts[:, enumerate_id] = data_group[i[3]].values

        cwts_mask = cwts_flag > 0.0
        cwts[:, enumerate_id][cwts_mask] = cwts[:, enumerate_id][cwts_mask] / cwts_flag[cwts_mask]
        cvis[:, enumerate_id][cwts_mask] = cvis[:, enumerate_id][cwts_mask] / cwts_flag[cwts_mask]
        cimwts[:, enumerate_id][cwts_mask] = cimwts[:, enumerate_id][cwts_mask] / cwts_flag[cwts_mask]
